...
export default class ComputerController extends Controller {
  ...
  async showLogDetails(computerName) {
    await this.showLastModDetails(computerName);
    try {
      const response = await fetch(`http://localhost:8080/backend_war_exploded/FetchUserLog?displayName=${computerName}`);
      if (!response.ok) {
      ...
      this.selectedLogGroup = [{ Message: 'Error fetching data', TimeCreated: '' }];
    }
  }
