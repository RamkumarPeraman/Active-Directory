...
void modify_attribute(LDAP* ldap_handle, const char* dn, const char* attribute, const char* new_value) {
    ...
    if(rc != LDAP_SUCCESS){
        cerr << "ldap_modify_ext_s error: " << ldap_err2string(rc) << std::endl;
    } 
    else {
       cout << "Attribute modified successfully." << std::endl;
    }
}
int main(int argc, char* argv[]) {
    if (argc != 4) {
        cerr << "Usage: " << argv[0] << " <CN> <attribute> <new_value>" << endl;
        return 1;
    }
    const char* cn = argv[1];
    const char* attribute = argv[2];
    const char* new_value = argv[3];
    string dn = string("CN=") + cn + ",CN=Computers,DC=zoho,DC=com";
    ldapBind();
