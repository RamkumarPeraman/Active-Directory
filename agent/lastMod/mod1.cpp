...
int main() {
    ...
    string executeCommandStr = "sshpass -p '" + password + "' ssh " + username + "@" + remoteHost + " powershell.exe -File C:/temp/script.ps1";

    string commandOutput = executeCommand(executeCommandStr);

    map<string, vector<Entry>> entries;
    istringstream output(commandOutput);
    string line;
    ...
    while(getline(output, line)) {
        ...
        if(line.find("objectName") != string::npos){
            ...
            if (validEntry){
                entries[modifiedTime].push_back({objectName, accountDomain, oldValue, newValue, modifiedTime, message, changedOn, organization});
                validEntry = false;
            }
        }
    }
