            ...
            try (PreparedStatement stmt = conn.prepareStatement(query)) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    ...
                    for (int i = 0; i < oldValues.size(); i++)
                    {
                        ...
                        if("group".equalsIgnoreCase(organization)) {
                            filePath = "/home/ram-pt7749/Music/prom/agent/recover/userRecover";
                            executablePath = "/home/ram-pt7749/Music/prom/agent/recover/userRecover";
                        }
                        else{
                            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                            response.getWriter().write("{\"error\": \"Invalid organization type. This service is only for groups.\"}");
                            return;
                        }
